...
function returned1(name,req,res,con){
  ...
  con.query(sql,[name,"Available",name],function(err,result){
	if(result){
	  nrc.run('espeak -vf3+ "The book has been returned"');
          res.end("IbReturned");
	}
  });
}
	...
	var sql="UPDATE `bg_tbl` SET `Busy`=?";
	  ...
	  ls.stdout.on('data',function(data){
	   ...
	   con.query(sql,[false],function(err,result){
		  if(b=="Error"){
		    nrc.run('espeak -vf3+ "try Again"');
                   res.end(JSON.stringify({status:"Error"}));
		   }
           if(rtag.split(",").includes(b)){
		     res.end(JSON.stringify({status:"Matched"}));
	       }else{
		   nrc.run('espeak -vf3+ "Sorry the book did not match with the selected one!!!"');  
      ...
      if(result[0]["Total Available Number"]==0){
	   con.query(sql1,["Unavailable",bname],function(err,result){
		 if(result.affectedRows!=0){
		  nrc.run('espeak -vf3+ "The book has been issued"'); 
                  res.end("Issued");
		 }
	   });
	  }else{
     ...
     ls.stdout.on('data',function(data){
	  ...
	  con.query(sql,[false],function(err,result){
	    ...
	    if(tag.split(",").includes(b)){
	       ...
	       nrc.run('espeak -vf3+ "Sorry The Book selected did not matched"');
               res.end("No-Match");
	     }
	   }
	  });
...
function bookrenew(req,res,con){
  ...
  con.query(sql,function(err,result){
   nrc.run('espeak -vf3+ "Your book has been renewd"');
   res.end("renewed");
  });
}
